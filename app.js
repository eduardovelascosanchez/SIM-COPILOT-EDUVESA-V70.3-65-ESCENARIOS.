...
function vitals(){if(!session)return;const v=session.vitals;$("hrValue").textContent=$("hrBottom").textContent=v.hr;$("spo2Value").textContent=$("spo2Bottom").textContent=v.spo2;$("bpValue").textContent=v.sys?`${v.sys}/${v.dia}`:"—/—";$("rrValue").textContent=v.rr;$("etco2Value").textContent=v.etco2;if($("etco2Bottom"))$("etco2Bottom").textContent=v.etco2;$("rhythmLabel").textContent=v.rhythm}
...
function bindDebriefNotes(){const instructor=$("debriefInstructorNotes"),commitment=$("debriefTeamCommitment");if(instructor)instructor.addEventListener("input",event=>{if(session?.debriefing)session.debriefing.instructorNotes=event.target.value});if(commitment)commitment.addEventListener("input",event=>{if(session?.debriefing)session.debriefing.teamCommitment=event.target.value})}
async function finish(){if(!session||finishing)return;finishing=true;session.running=false;clearInterval(timerId);toast("Cerrando caso y preparando debriefing…");let voiceData={entries:[],text:"",audioUrl:"",recognitionSupported:false};try{voiceData=await voiceController?.stopAndFinalize?.()||voiceController?.getData?.()||voiceData}catch{}session.voiceData=voiceData;const r=calculateResult(session);session.debriefing=buildStructuredDebrief(session,r,voiceData);$("finalScore").textContent=r.score;$("scoreRing").style.background=`conic-gradient(var(--primary) ${r.score*3.6}deg,#173244 0deg)`;$("resultTitle").textContent=r.decision;$("resultSummary").textContent=`${session.completed.size} de ${session.scenario.goals.length} objetivos completados. Debriefing generado con ${session.debriefing.metrics.voiceUtterances} frases transcritas y ${session.log.length} eventos.`;$("decisionBadge").textContent=session.instructorDecision?"Decisión del instructor":"Evaluación automática";$("debriefing").innerHTML=renderStructuredDebrief(session.debriefing,session,voiceData);$("metricGrid").innerHTML=`<div>Puntuación<br><b>${r.score}</b></div><div>Duración<br><b>${formatTime(session.elapsed)}</b></div><div>Errores críticos<br><b>${session.criticalErrors}</b></div>`;bindDebriefNotes();view("resultView");finishing=false}
