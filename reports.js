...
function debriefText(session){
  ...
  return[
    ...
    ...d.opportunities.map(item=>`- ${item}`),
    "",
    "COMUNICACIÓN Y TRABAJO EN EQUIPO",
    ...d.communicationFindings.map(item=>`- ${item}`),
    "",
    "PREGUNTAS GUÍA",
    ...d.questions.map(item=>`- ${item}`),
    "",
    "MENSAJES PARA LLEVAR",
    ...d.takeaways.map(item=>`- ${item}`),
    "",
    ...
    d.transcriptEvidence||"Sin transcripción automática."
  ];
}
export function downloadReport(session,result){
  const lines=[
    "SIMCOPILOT EDUVESA V7.0.8",
    "Reporte completo de simulación y debriefing",
    "",
