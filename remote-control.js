...
function friendlyError(err,context="connect"){
  ...
  if(type==="network"||type==="server-error"||type==="socket-error")return "No fue posible enlazar los dispositivos. Revisa Internet y abre la página directamente en Safari o Chrome, no dentro de WhatsApp.";
...
function createPeer(id){
  return new window.Peer(id,{debug:1,config:{iceServers:[{urls:"stun:stun.l.google.com:19302"},{urls:"stun:stun1.l.google.com:19302"}]}});
}

export async function createMonitorRoom(attempt=0){
  try{await loadPeerJS()}catch{setStatus(friendlyError({type:"browser-incompatible"}),"error");return false}
  clearConnectionTimers();
  if(connection){try{connection.close()}catch{}connection=null}
  if(peer&&!peer.destroyed)peer.destroy();
  roomId=randomRoom();
  role="monitor";
  setStatus("Creando sesión segura…","pending");
  peer=createPeer(roomId);
  peer.on("open",async id=>{
    roomId=id;
    const code=document.getElementById("remoteRoomCode");
    if(code)code.textContent=id;
    setStatus("Esperando instructor. Mantén esta pantalla abierta.","waiting");
  ...
  peer.on("error",err=>{
    ...
    setStatus(friendlyError(err,"monitor"),"error");
  });
  return true;
}
